...
RSpec.describe Secp256k1::EllSwift do
  ...
  let(:packet_encoding_vectors) { read_csv('packet_encoding_test_vectors.csv') }
  ...
  describe 'test vectors' do
    it do
      ...
      packet_encoding_vectors.each do |v|
        ...
        our_private_key = v['in_priv_ours']
        our_ell = v['in_ellswift_ours']
        their_ell = v['in_ellswift_theirs']
        shared_x = target.ellswift_ecdh_xonly(their_ell, our_ell, our_private_key, initiating)
